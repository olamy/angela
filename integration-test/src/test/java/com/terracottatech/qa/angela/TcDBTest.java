...
public class TcDBTest {
  ...
  public void testConnection() throws Exception {
    TcConfig tcConfig = new TcConfig(version("10.2.0-SNAPSHOT"), "/terracotta/10/tc-config-a.xml");

    Topology topology = new Topology("1", Distribution.distribution(version("10.2.0-SNAPSHOT"), PackageType.KIT, LicenseType.TC_DB), tcConfig);
    ...
    try (TsaControl control = new TsaControl(topology, license)) {
      ...
      Future<Void> f = control.clientControl().submit("localhost", () -> {
            try {
