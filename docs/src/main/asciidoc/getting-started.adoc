...
<4> Specify the Terracotta distribution : version, package type (KIT) and License (TC DB)
...
----
<1> Specify the TSA cluster topology
<2> Specify the Terracotta cluster config with security
<3> Specify the SecurityRootDirectory for "Server1"
<4> Create a Tsa logical instance that serves as an endpoint to call functionalities regarding the Tsa lifecycle
<5> Install the Tsa from the distribution on the appropriate server(s) (localhost in this case)
<6> Install the license with the cluster tool using SecurityRootDirectory
